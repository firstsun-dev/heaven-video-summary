#!/usr/bin/env bash
# Store the remote from env if it exists
ENV_REMOTE="${RCLONE_REMOTE:-}"

source "$(dirname "$0")/common.sh"

# Restore from env if common.sh/config.env overrode it
[[ -n "$ENV_REMOTE" ]] && RCLONE_REMOTE="$ENV_REMOTE"
