...
if [[ -n "${RCLONE_REMOTE:-}" ]]; then
    ...
    # 2. Sync all .vtt subtitle files (keeping directory structure)
    echo "📤 Syncing .vtt subtitle files..."
    rclone copy "$ROOT_DIR/$ARCHIVE_DIR" "$RCLONE_REMOTE" --include "*.vtt" --include "*/"
fi
printf "\n=== Stage 4 complete ===\n"
